"e""
This script searches through ../DATA for all recent calibrations of boards with ip in range(100,130). It then makes .sh scripts which it runs with sbatch. Note that if any files that are added to ../DATA which are not in the ../DATA/name_to_date.txt file must be added manually to the search (i.e. folder 2017-08-09 below).
)"""
import os
import datetime
import subprocess
...
def make_script(command_string, ix):
    ...
    with open('rec_' + str(ix) + '.sh', 'w') as writer:
        writer.write(
"""#!/bin/bash                                                                                                                                                                         
#                                                                                                                                                                                   
...
folder_name = '/n/atlasfs/atlasdata/cosmic_ray/most_recent_calibrations_as_of_2017_08_10'           
...
for enum,ip in enumerate(recent_xadc):
    ...
    subprocess.Popen('sbatch rec_' + str(2*enum) + '.sh', shell=True)
    ...
    subprocess.Popen('sbatch rec_' + str(2*enum+1) + '.sh', shell=True)
    # sleep for a second to be polite to the sbatch scheduler
    sleep(1)
